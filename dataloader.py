...
AUDIO_DIR = "/Users/francesco/Desktop/audio2midi/guitarset/audio_hex-pickup_debleeded/"
...
class GuitarSetDataset(data.Dataset):
    def __init__(self, audio_dir=AUDIO_DIR, jams_dir=JAMS_DIR, apply_nmf=False, apply_sections=True):
        
        #added flag to decide whether to apply NMF
    ...
    def _get_file_pairs(self):
        ...
        '''returns a list of elements (audio_path, jams_path)'''
        ...
        for jams_file in jams_files:
            ...
            matching_wav = [f for f in os.listdir(self.audio_dir) if f.startswith(prefix) and f.endswith(".wav")]
            if matching_wav:
                samples.append((os.path.join(self.audio_dir, matching_wav[0]), os.path.join(self.jams_dir, jams_file)))
        return samples
    ...
    def _load_audio(self, file_path):
        ...
        #NMF
        if self.apply_nmf:
            CQT_norm = self.__class__.nmf(CQT_norm)
    ...
    def __getitem__(self, idx):
        ...
        if self.apply_sections:
            # Split the CQT and piano roll into overlapping sections
